...
function ApplySkin(skin, clothes)
	...
	SetPedHeadOverlay			(playerPed, 11,		Character['bodyb_1'],			(Character['bodyb_2'] / 10) + 0.0)			-- Body Blemishes + opacity
		...
		local j = 0
		for i = 75,94 do
			SetPedFaceFeature(GetPlayerPed(-1), j, Character[i]);						--"Nose Width",               // 0
			j = j + 1
		end
...
AddEventHandler('skinchanger:loadSkin', function(skin, cb)
	if skin['model_hash'] == GetHashKey("mp_m_freemode_01") or skin['model_hash'] == GetHashKey("mp_f_freemode_01") then
		isOtherPed = false
		...
		isOtherPed = true
		RequestModel(skin['model_hash'])
		while not HasModelLoaded(skin['model_hash']) do
			Citizen.Wait(0)
		end
		SetPlayerModel(PlayerId(), skin['model_hash'])
